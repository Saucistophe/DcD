import gameData from './assets/missions/medieval.yml';
...
function mapTo2dArray(inputMap)
{
  ...
  for(let lineIndex in currentLines)
  {
    ...
    {
      if(!currentMap[charIndex])
        currentMap[charIndex] = [];
  ...
  for(let mission of gameData.missions)
  {
    ...
    for(let card of mission.cards)
    {
      ...
      for(let paragraphIndex in card.paragraphs)
      {
        ...
        if(numberOfClosingSpans > numberOfOpeningSpans)
          alert(`Too many closing tabs for paragraph ${paragraphIndex} in card ${card.title}`);

        // Put in opening and closing tags.
        paragraph = paragraph.replace(/<(\w*)>/g, '<span script="$1">');
        paragraph = paragraph.replace(/<\/>/g, '</span>');
        for(let i = numberOfOpeningSpans; i < numberOfClosingSpans; i++)
          paragraph += '</span>';
      ...
      if(currentMap)
      {
        ...
        if(currentCardY < height - 1
				&& currentMap[currentCardX][currentCardY+1].match(/[a-zA-Z]/i))
          card.top = currentMap[currentCardX][currentCardY+1];
        // Right
        if(currentCardX < width - 1
				&& currentMap[currentCardX+1][currentCardY].match(/[a-zA-Z]/i))
          card.right = currentMap[currentCardX+1][currentCardY];
        // Bottom
        if(currentCardY > 0
				&& currentMap[currentCardX][currentCardY-1].match(/[a-zA-Z]/i))
          card.bottom = currentMap[currentCardX][currentCardY-1];
        // Left
        if(currentCardX > 0
				&& currentMap[currentCardX-1][currentCardY].match(/[a-zA-Z]/i))
          card.left = currentMap[currentCardX-1][currentCardY];
      }

      // Process confidentiality thingies.
      if(card.confidential)
      {
        if(card.confidential.includes('♂'))
          card.confidential = 'him';
        else if(card.confidential.includes('♀'))
          card.confidential = 'her';
        else if(card.confidential.includes('⚤'))
          card.confidential = 'both';
      }
    }
